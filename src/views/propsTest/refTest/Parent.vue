<template>
  <div>
    <h1 class="white--text">Ref TEST</h1>
    <Child01 v-if="true" :item="test01.refObj.value" />
    <Child02 v-if="true" :item="test01.refObj.value" />
    ...
    <Child03 v-if="true" :item="test03.refObj.value" />

    <Child04 v-if="true" :item="test04.objArray.value" />
    <Child05 v-if="true" :item="test05.objArray.value" />
    <Child06 v-if="true" :item="test06.objArray.value" />
    <Child07 v-if="true" :item="test07.deepObj.value" />
  </div>
</template>
...
const defineTest01 = () => {
  ...
  const changeData = (time = 2000): void => {
    //オブジェクトの値を変更する
    setTimeout(() => {
      refObj.value.id = "newVal"
      refObj.value.name = "newVal"
    }, time)
  }

  const test01 = { refObj, changeData }
  return test01
}
...
const defineTest04 = () => {
  ...
  const changeData = (time = 2000): void => {
    //配列内の1番目要素の値を変更する
    setTimeout(() => {
      objArray.value[0].id = "newVal"
      objArray.value[0].name = "newVal"
    }, time)
  }
...
const defineTest05 = () => {
  ...
  const changeData = (time = 2000): void => {
    ...
    setTimeout(() => {
      objArray.value[0] = { id: "newVal", name: "newVal" }
    }, time)
  }
...
const defineTest06 = () => {
  ...
  const changeData = (time = 2000): void => {
    ...
    setTimeout(() => {
      objArray.value.push({ id: "newVal", name: "newVal" })
    }, time)
  }
...
const defineTest07 = () => {
  ...
  const deepObj = ref<DeepItem>({ id: "1", name: "john", deep: obj })

  const changeData = (time = 2000): void => {
    //オブジェクト内のオブジェクトの値を変更する
    setTimeout(() => {
      deepObj.value.deep.title = "newValue"
    }, time)
  }
